...
function createApidocSwagger(options) {
    ...
    try {
        ...
        if (app.options.parse !== true){
            ...
            var definitions = options.definitions ? JSON.parse(fs.readFileSync(options.definitions) || '') : null;
            api.swaggerData = JSON.stringify(apidocSwagger.toSwagger(apidocData , projectData, definitions));
            if (options.str) { return api.swaggerData }
